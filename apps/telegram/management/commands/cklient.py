...
from apps.telegram.models import Booking, Business, Cklient, UserCklient, UserBusiness
from apps.telegram.management.commands.bot_instance import bot
from aiogram.utils.exceptions import MessageToDeleteNotFound  # Правильный импорт
import json
...
async def search_action(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        # Сохранение запроса клиента в базу данных асинхронно
        ...
        # Отправка запроса бизнес-клиентам асинхронно
    ...
    except UserCklient.DoesNotExist:
        logging.error(f"Пользователь с id={callback_query.from_user.id} не найден.")
        await callback_query.answer("Ошибка: Пользователь с таким id не найден.", show_alert=True)
    except Exception as e:
        logging.error(f"Ошибка при сохранении бронирования: {e}")
        await callback_query.answer("Ошибка при обработке запроса.", show_alert=True)
...
async def handle_accept(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        client_user = await sync_to_async(UserCklient.objects.get)(user_id=client_id)
        ...
        # Use sync_to_async for Booking creation
        accepted_by_user = await sync_to_async(UserBusiness.objects.get)(user_id=callback_query.from_user.id)
        await sync_to_async(create_booking)(
            business_client=business_client,
            client_user=client_user,
            accepted_by_user=accepted_by_user,
        )
        logging.info(f"Бронирование сохранено: {business_client.user.username} забронировал для {client_user.username} (принято пользователем {accepted_by_user.username})")

    except Business.DoesNotExist:
        logging.error(f"Business с id={business_id} не найден.")
        await callback_query.answer("Ошибка: Бизнес с таким id не найден.", show_alert=True)
    except UserCklient.DoesNotExist:
        logging.error(f"UserCklient с id={client_id} не найден.")
        await callback_query.answer("Ошибка: Пользователь с таким id не найден.", show_alert=True)
    except Exception as e:
        logging.error(f"Ошибка при сохранении бронирования: {e}")
        await callback_query.answer("Ошибка при обработке запроса.", show_alert=True)

# Define a synchronous function to create booking
def create_booking(business_client, client_user, accepted_by_user):
    Booking.objects.create(
        business_user=business_client.user,
        client_user=client_user,
...
async def handle_decline(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logging.warning(f"Message to delete not found for chat {callback_query.message.chat.id} and message {callback_query.message.message_id}")
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(go_back, Text(equals='choose_date'), state='*')
